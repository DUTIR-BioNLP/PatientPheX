...
PatientPheX数据集共包含200篇全文级PMC生物医学文献，其中训练集60篇、开发集（A榜测试集）20篇、测试集（B榜测试集）120篇。数据集共标注506位患者、15,193个表型实体和2,874个唯一HPO概念，并标注了否定表型及复合表型等复杂现象。其中开发集和测试集不包含标准答案，需在线提交预测结果。
